...
def push_process_raw(host, log, rule_hit_name, score, chain_hash, type, parent_pid, target_pid, self_hash, target_image_path, target_hash):
    global g_engine
    global g_rawdata_table
    global g_rawdata_table_ins
    ...
    ins = g_rawdata_table_ins.values(host=host,
                                     ...
                                     target_image_path=log['Data']['TargetFilename'] if 'TargetFilename' in log['Data'] else '',
...
def push_threat_log(host, risk_score, hit_rule_json, process_chain_hash, raw_json, type, start_process_info):
    ...
    ins = g_threat_table_ins.values(
        host=host, risk_score=risk_score, process_chain_hash=process_chain_hash, hit_rule=hit_rule_json, type=type, data=raw_json, timestamp=int(round(time.time() * 1000)), is_end=0, start_process_info=start_process_info, handle_type=0)
    # 连接引擎
    conn = g_engine.connect()
    # 执行语句
    result = conn.execute(ins)
